...
window.addEventListener('game.response', (e) => {
  ...
  if (path === '/kcsapi/api_req_kousyou/createitem') {
    if (body.api_create_flag === 0) {
      setTimeout(window.warn, devResultDelay, i18next.t('main:DevelopFailed'))
      ...
      const name = _((body as unknown as APIReqKousyouCreateitemResponse).api_get_items)
        ...
        .join(' | ')
      setTimeout(window.success, devResultDelay, i18next.t('main:DevelopSuccess', { name }))
    }
  }
  ...
  if (path === '/kcsapi/api_req_kousyou/remodel_slot') {
    ...
    if (body.api_remodel_flag === 0) {
      setTimeout(window.warn, improveResultDelay, i18next.t('main:ImproveFailed', { name }))
    } else if (body.api_remodel_flag === 1) {
      setTimeout(window.success, improveResultDelay, i18next.t('main:ImproveSuccess', { name }))
    }
  }
})
