...
StatusLabel = React.createClass
  ...
  render: ->
    if @props.label? and @props.label == 0
      ...
      <OverlayTrigger placement="top" overlay={<Tooltip id="statuslabel-status-2">{__ 'Ship tag: %s', 'E1, E2, E7'}</Tooltip>}>
        <Label bsStyle="info"><FontAwesome key={0} name='tag' /></Label>
      </OverlayTrigger>
    else if @props.label? and @props.label == 3
      <OverlayTrigger placement="top" overlay={<Tooltip id="statuslabel-status-3">{__ 'Ship tag: %s', 'E3, E6, E7'}</Tooltip>}>
        <Label bsStyle="primary"><FontAwesome key={0} name='tag' /></Label>
      </OverlayTrigger>
    else if @props.label? and @props.label == 4
      <OverlayTrigger placement="top" overlay={<Tooltip id="statuslabel-status-4">{__ 'Ship tag: %s', 'E4, E7'}</Tooltip>}>
        <Label bsStyle="success"><FontAwesome key={0} name='tag' /></Label>
      </OverlayTrigger>
    else if @props.label? and @props.label == 5
      <OverlayTrigger placement="top" overlay={<Tooltip id="statuslabel-status-5">{__ 'Ship tag: %s', 'E5'}</Tooltip>}>
