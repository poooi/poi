...
PaneBodyMini = React.createClass
  ...
  handleResponse: (e) ->
    ...
    updateflag = false
    switch path
      when '/kcsapi/api_port/port', '/kcsapi/api_req_hensei/change', '/kcsapi/api_req_hokyu/charge', '/kcsapi/api_req_map/next', '/kcsapi/api_req_kaisou/remodeling', '/kcsapi/api_req_nyukyo/speedchange'
        updateflag = true
        label = @updateLabels()
        ...
        for shipId, j in @props.deck.api_ship
          ...
          <div key={j} className="ship-tile">
            <OverlayTrigger placement={if (!window.doubleTabbed) && (window.layout == 'vertical') then 'left' else 'right'} overlay={
              <Popover id="ship-pop-#{@props.key}-#{j}" className="ship-pop">
