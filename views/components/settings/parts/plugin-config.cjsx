    ...
    <form>
      ...
      <Grid>
      {
        plugins.map (plugin, index) =>
          <Col key={index} xs={12}>
            <Input type="checkbox" label={[plugin.displayName, ' @ ', plugin.author || 'Unknown', '：', plugin.description, <br />, 'Version ',  plugin.version || '1.0.0']} checked={@state.enabled[index]} onChange={@handleChange.bind @, index } />
          </Col>
      }
      </Grid>
    </form>
