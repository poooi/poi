...
if config.get('poi.first', '0.0.0') != POI_VERSION
  ...
  content =
    <div>
      ...
      <p style={color: 'red'}>poi 不会修改任何游戏内的发包与收包，并且不提供给插件修改发包与收包的能力。但是请注意，请使用可信的 poi 版本和可信的插件！</p>
      ...
      <p>poi 有的时候会出现flash无法正常缩放的情况，多拖拖窗口，或者过一会儿拖拖窗口。这个功能依赖于网速，所以多多包容……</p>
      <p>poi 对小分辨率不是特别友好……建议在小分辨率屏幕下使用横版，大分辨率下使用纵版防止缩放过度。</p>
