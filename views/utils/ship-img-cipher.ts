// Magic of tanaka — the resource table used to derive kcs2 asset URLs (ship
// banner/remodel/card, equip icons, etc.). This is the single source of
// truth: views/utils/ship-img.ts uses it to build in-app asset URLs, and
// fcd/gen-shipavatar.js (a standalone Node script outside the app's build)
// loads this same file via babel-hook.js so its downloader can never drift
// from what the game client actually serves.
...
export function createKey(t: string): number {
  let e = 0
  if (null != t && '' != t) {
...
export function createCipher(id: number | string, seed: string): string {
