...
import { hasAtLeast, hasSome, shipIdIs, taisenAbove } from './combinators'
import {
  equipTais,
  equipTaisAbove,
  isASWAircraft,
  ...
  isSonar,
  isTorpedoBomber,
} from './equip-predicates'
import { isFubukiK3Go, isHyuuGaK2, isIsuzuK2, isKai, isTatsutaK2 } from './ship-predicates'
...
const isFletcherClassKai = (ship: GameShip) => ship.api_ctype === 91 && isKai(ship)
// NOTE: intentionally broader than the AACI isFletcherClassOrKai (ctype-based
// so future Fletcher-class Kai are covered); keep them separate.
...
export const isOASW: OASWPredicate = overSome(
  ...
  overEvery(
    ...
    overSome(
      ...
      // オートジャイロ二機
      overEquips(hasAtLeast(isAutogyro, 2)),
    ),
  ),
  // 神州丸改 大和改二重
  overEvery(
