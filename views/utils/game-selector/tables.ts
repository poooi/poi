...
export interface EquipFilterCategory {
  ...
  details: number[]
  /**
   * Representative equipment icon (`api_type[3]`) for the dropdown. Omitted for
   * 全装備, which stands for no particular gear — poi's `-1` placeholder would
   * put a tortoise beside it.
   */
  icon?: number
}
...
export const mergeSelectorTables = (
  ...
  return {
    ...
    filteringDetailCategories: mergeRecord(
      tables.filteringDetailCategories,
      DEFAULT_SELECTOR_TABLES.filteringDetailCategories,
    ),
    equipTypeSpOverrides: mergeRecord(
      tables.equipTypeSpOverrides,
      DEFAULT_SELECTOR_TABLES.equipTypeSpOverrides,
    ),
    // The only field replaced outright: these rules are matched positionally on
    // (types, icon), with no key to merge on.
    filterTypeSplits: nonEmpty(tables.filterTypeSplits) ?? DEFAULT_SELECTOR_TABLES.filterTypeSplits,
    pickerTypeOverrides: mergeRecord(
...
/**
 * Why every field merges rather than replaces:
 *
 * a payload can be *older* than the code. poi only refreshes fcd when the
 * remote version is newer, so a copy cached before an entry or key existed
 * would otherwise win wholesale and blank it out. Delivered values always win
 * where they overlap; the defaults only fill gaps.
 *
 * The cost is that fcd can correct an entry but cannot delete one. That is the
 * right way round here: the payload is generated from these defaults, so a
 * missing entry means the payload predates it, not that the game dropped it.
 */
const mergeEntries = <T extends { id: number }>(delivered: T[] | undefined, defaults: T[]): T[] => {
  ...
  const missing = new Map(defaults.map((entry) => [entry.id, entry]))
  const merged = entries.map((entry) => {
    const fallback = missing.get(entry.id)
    missing.delete(entry.id)
    return fallback ? { ...fallback, ...entry } : entry
  })
  // Entries this build added that the payload predates keep their defaults.
  return [...merged, ...missing.values()]
}

const mergeRecord = <V>(
  delivered: Record<number, V> | undefined,
  defaults: Record<number, V>,
): Record<number, V> => (delivered ? { ...defaults, ...delivered } : defaults)
