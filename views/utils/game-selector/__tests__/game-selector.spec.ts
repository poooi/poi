...
describe('an equipment category that accepts nothing', () => {
  // Matching the ship and land base paths, which both yield an empty list for
  // an unknown tab rather than falling through to everything.
  spec('yields an empty list, not the whole inventory', () => {
    expect(buildEquipList(allEquipEntries, equipFilter({ category: 999 }))).toHaveLength(0)
    expect(buildEquipList(allEquipEntries, equipFilter({ category: 4, detail: 999 }))).toHaveLength(
      0,
    )
  })
