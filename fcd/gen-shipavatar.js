...
// Magic of tanaka, kept in sync with views/utils/ship-img.ts
const resource = [
...
const createKey = (seed) => [...seed].reduce((sum, c) => sum + c.charCodeAt(0), 0)

const createCipher = (id, seed) =>
  (
    ((17 * (id + 7) * resource[(createKey(seed) + id * seed.length) % 100]) % 8973) +
    1000
  ).toString()
